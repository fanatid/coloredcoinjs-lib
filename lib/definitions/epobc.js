...
var ZERO_HASH = require('../util/const').ZERO_HASH
 ...
 */
EPOBCColorDefinition.fromDesc = function (desc, resolver, opts) {
  return Promise.try(function () {
    ...
    return resolver.resolve(desc, _.extend({autoAdd: true}, opts))
  })
}
 ...
 * @return {Promise.<?EPOBCColorDefinition>}
 */
EPOBCColorDefinition.fromTx = function (tx, resolver, opts) {
  return Promise.try(function () {
...
function (tx, inColorValues, getTxFn) {
  ...
  return Promise.try(function () {
    ...
    if (tag === null || tag.isGenesis()) {
      var outColorValues = jsUtil.getArrayOfNull(tx.outputs.length)

      var isValidColorTx = tag !== null &&
                           self.isGenesis(tx) &&
                           tx.outputs.length > 0 &&
                           tx.outputs[0].satoshis - tag.getPadding() > 0

      if (isValidColorTx) {
        var value = tx.outputs[0].satoshis - tag.getPadding()
        outColorValues[0] = new ColorValue(self, value)
      }
 ...
 */
EPOBCColorDefinition._getXferAffectingInputs = function (ftx, padding, oidx) {
  return Promise.try(function () {
    var tx = ftx.getTx()
    ...
    var outValueWop = tx.outputs[oidx].satoshis - padding
    if (outValueWop <= 0) {
      return []
    }
    ...
    return Promise.reduce(tx.inputs, function (affectingInputs, input, index) {
      if (isBroken) {
 ...
 * @return {Promise.<number[]>}
 */
EPOBCColorDefinition.getAffectingInputs = function (tx, oidxs, getTxFn) {
  return Promise.try(function () {
    var tag = Tag.fromTx(tx)
    if (tag === null || tag.isGenesis()) {
      return []
    ...
    return ftx.ready
      .then(function () {
        var padding = tag.getPadding()
        return Promise.map(oidxs, function (oidx) {
          return EPOBCColorDefinition._getXferAffectingInputs(ftx, padding, oidx)
        })
