...
ComposedTx.prototype.addInputs = function (inputs) {
  ...
  inputs.forEach(function (input) {
    self._inputs.push({txid: input.txid, vout: input.vout})
...
ComposedTx.prototype.setInputSequence = function (index, sequence) {
  if (index < 0 || index >= this._inputs.length) {
    throw new RangeError('Input for index not found')
  }
...
/**
 * @param {(ComposedTx~OutTarget|ComposedTx~OutScriptValue)} output
 */
...
/**
 * @param {Array.<(ComposedTx~OutTarget|ComposedTx~OutScriptValue)>} outputs
 */
