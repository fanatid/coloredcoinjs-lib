...
var SQL = {
  create: {
    ...
    indexOutput: 'CREATE INDEX IF NOT EXISTS data_idx ON data (txid, oidx)'
  },
  insert: {
    row: 'INSERT INTO data (txid, oidx, colorId, value) VALUES ($1, $2, $3, $4)'
  },
  select: {
    all: 'SELECT oidx, colorId, value FROM data WHERE txid = $1',
    by: {
      oidx: 'SELECT colorId, value FROM data WHERE txid = $1 AND oidx = $2',
      colorId: 'SELECT oidx, value FROM data WHERE txid = $1 AND colorId = $2',
      both: 'SELECT value FROM data WHERE txid = $1 AND oidx = $2 AND colorId = $3'
    }
  },
  delete: {
...
AbstractSyncColorDataStorage.prototype.add = function (data) {
  return this._provider.transaction(function (tx) {
    ...
    return tx.execute(SQL.select.by.both, [data.txid, data.oidx, data.colorId])
      .then(function (rows) {
        if (rows.length > 0) {
        ...
        // save
        return tx.execute(SQL.insert.row, [data.txid, data.oidx, data.colorId, data.value])
      })
  })
  .then(function () {
    // return new record
...
AbstractSyncColorDataStorage.prototype.get = function (opts) {
  return this._provider.transaction(function (tx) {
    if (opts.oidx !== undefined && opts.colorId !== undefined) {
      return tx.execute(SQL.select.by.both, [opts.txid, opts.oidx, opts.colorId])
        .then(function (rows) {
    ...
    if (opts.oidx !== undefined) {
      return tx.execute(SQL.select.by.oidx, [opts.txid, opts.oidx])
        .then(function (rows) {
    ...
    return tx.execute(SQL.select.all, [opts.txid])
      .then(function (rows) {
        ...
        rows.forEach(function (row) {
          var ovalues = result[row.oidx] || {}
          ovalues[row.colorId] = row.value
          result[row.oidx] = ovalues
        })
        return result
      })
  })
