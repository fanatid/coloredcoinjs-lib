...
var oddStorage = require('odd-storage')(Promise)
...
function AbstractSyncColorDataStorage () {
  ...
  self._SQL = SQL['SQLite']
  if (!!oddStorage.PostgreSQL &&
      self._storage instanceof oddStorage.PostgreSQL) {
    self._SQL = SQL['PostgreSQL']
  }

  self._storage.open()
    .then(function () {
      return self._storage.withLock(function () {
        return self._storage.executeSQL(self._SQL.create.tables.tx)
          .then(function () {
            return self._storage.executeSQL(self._SQL.create.tables.values)
          })
          .then(function () {
